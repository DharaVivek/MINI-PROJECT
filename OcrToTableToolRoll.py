...
class OcrToTableToolRoll:
    ...
    def crop_each_bounding_box_and_ocr(self):
        ...
        for row in self.rows:
            for bounding_box in row:
                x, y, w, h = bounding_box
                y = y - 5
                # print(image_number, x, y, w, h)
                cropped_image = self.original_image[y:y+h, x:x+w]
                image_slice_path = "./ocr_slices_roll/img_" + str(image_number) + ".jpg"
    ...
    def store_process_image(self, file_name, image):
        path = "./process_images_roll/ocr_table_tool/" + file_name
